...
const HighlightedTags: FC<Iprops> = ({title, tags}) => {
  ...
  return (
    <>
      <Heading fontWeight="normal" fontSize={24} mt={12} mb={4}>{title}</Heading>
      <Stack direction="row" spacing={16} pb={6}>
        {tags.map(t => {
          const {slug, name, id, category} = t;
          const Icon: React.LazyExoticComponent<FontAwesomeIconProps> = (props) => <FontAwesomeIcon icon={iconTag[slug] || 'envelope'} {...props}/>;
          return (
