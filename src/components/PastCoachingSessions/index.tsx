...
const PastCoachingSessions: FC<Iprops> = ({currentUser}) => {
  ...
  useEffect(() => {
    async function loadPast() {
      const u = await axios.get(`${envConfig.API_URL}/v1/user/past`, {params: {id: currentUser.id}});
      setPast(u.data);
    }
  ...
  return (
    <>
    <Heading fontWeight="normal" fontSize={24} mt={12}>Past Coaching Sessions</Heading>
    <Box
        display="flex"
        flexFlow="row wrap"
        gap={4}
    ...
    {past.map(b => {
      ...
      return <Coach key={booking.id} coachInfo={coach} booking={booking} currentUser={currentUser} />
    })}
    </Box>
    </>
  )
